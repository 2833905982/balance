...
//		Delay_ms(50);
		...
		B = MPU_Get_Accelerometer(&aacx,&aacy,&aacz);	//得到加速度传感器数据
		C = MPU_Get_Gyroscope(&gyrox,&gyroy,&gyroz);  //得到陀螺仪的数据
		A = mpu_dmp_get_data(&pitch,&roll,&yaw);			//得到欧拉角（姿态角）
		if(yaw<0)		
		{
